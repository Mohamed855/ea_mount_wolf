                ...
                <div class="row text-center">
                    <div class="col-12 col-lg-8 m-auto">
                        <div class="overflow-scroll border bg-white shadow rounded-2 py-5 px-4 px-lg-5">
                            <h3 class="pb-4">@lang('panel.edit') @lang('translate.aboutUs')</h3>
                            <form action="{{ route('about.update', $about->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="col-md-5 col-12 d-inline-block">
                                    <div class="pb-3">
                                        <textarea name="title" id="" rows="4" class="form-control py-2" placeholder="@lang('panel.title')">{{ $about->title }}</textarea>
                                    </div>
                                </div>
                                <div class="col-md-5 col-12 d-inline-block">
                                    <div class="pb-3">
                                        <textarea name="text" id="" rows="4" class="form-control py-2" placeholder="@lang('panel.text')">{{ $about->text }}</textarea>
                                    </div>
                                </div>
                                <div class="col-md-10 col-12 m-auto">
                                    <div class="pb-3">
                                        <input type="file" name="image" id="formFile" class="form-control py-2" value="{{ $about->image }}"  placeholder="@lang('panel.image')">
                                    </div>
                                </div>
                                <div class="col-md-10 col-12 m-auto">
