                ...
                <div class="row text-center">
                    <div class="col-12 col-lg-8 m-auto">
                        <div class="overflow-scroll border bg-white shadow rounded-2 py-5 px-4 px-lg-5">
                            <h3 class="pb-4">@lang('panel.edit') @lang('panel.link')</h3>
                            <form action="{{ route('social-links.update', $socialLink->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="col-md-10 col-12 d-inline-block">
                                    <div class="pb-3">
                                        <input type="text" name="name" class="form-control py-2" value="{{ $socialLink->name }}" placeholder="@lang('panel.name')">
                                    </div>
                                    <div class="pb-3">
                                        <input type="text" name="link" class="form-control py-2" value="{{ $socialLink->link }}" placeholder="@lang('panel.link')">
                                    </div>
                                    <div class="pb-3">
                                        <input type="text" name="icon" class="form-control py-2" value="{{ $socialLink->icon }}" placeholder="font-awesome icon">
                                    </div>
                                </div>
