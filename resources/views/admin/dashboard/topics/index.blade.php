                    ...
                    <div class="col-lg-12">
                        <table class="table mb-5 bg-white">
                            ...
                            @foreach($topics as $topic)
                                <tr>
                                    <td><img src="{{ asset('images/topics/' . $topic->image ) }}" style="max-width: 100px; height: 60px;">
                                    </td><td>{{ $topic->title }}</td>
                                    ...
                                    <td>
                                        <a href="{{ route('ea_topics.edit', $topic->id) }}" class="btn btn-outline-primary btn-sm btn-rounded">
                                            Edit
                                        </a>
                                        <a href="{{ route('topic', $topic->id) }}" class="btn btn-outline-warning btn-sm btn-rounded">
                                            View
                                        </a>
                                        <form action="{{ route('toggle_publish_topic', $topic->id) }}" method="post" class="d-inline">
                                            @csrf
