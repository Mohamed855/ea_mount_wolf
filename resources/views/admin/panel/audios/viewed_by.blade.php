...
@section('panel_title')
    Views Details
    <a href="{{ route('report.download', ['id' => $audio_id, 'table' => 'audio_views']) }}" class="btn btn-outline-success mx-3">Download Report</a>
@endsection
...
@section('panel_content')
    ...
    <div class="scroll-bar overflow-scroll">
        <table class="table bg-white">
            ...
            <tbody>
                @if(count($audio_user_views->get()) > 0)
                    @if(isset($_GET['search']))
                        @php($audio_user_views = $audio_user_views->where('users.first_name', 'like', '%' . $_GET['search'] . '%')
                    ...
                    @else
                        @php($audio_user_views = $audio_user_views->get())
                    @endif
                    @if(count($audio_user_views) > 0)
                        @foreach($audio_user_views as $user)
                            <tr>
