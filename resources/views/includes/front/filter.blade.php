<form method="get" class="controle row pb-5">
    <div class="col-md-3">
        <select name="filter" onchange="this.form.submit()" class="form-control">
            <option value="none" {{ !isset($_GET['filter']) ? 'selected' : '' }} disabled>Group by</option>
    ...
    <div class="col-md-3">
        <div class="date">
                <input type="date" onchange="this.form.submit()" name="date" value="{{ isset($_GET['date']) && DateTime::createFromFormat('Y-m-d', $_GET['date']) ? $_GET['date'] : 'mm/dd/yyyy' }}" class="form-control">
        </div>
    </div>
</form>
